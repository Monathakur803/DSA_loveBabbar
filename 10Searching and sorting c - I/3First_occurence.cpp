    ...
    int ans = -1;  // yaha p ans = -1 isliye hai q ki agar target value available hi na ho to -1 return krega
     ...
     while (s <= e){
        if (arr[mid] == target){
            ...
            e = mid-1; // yaha hum left me chle gye
        }
...
int main()
{
    vector<int> arr{1,3,3,3,4,4,4,4,4,6,7};
    int target = 4;

    int indexOfFirstOccurence = firstOccurence(arr, target);

    cout<< " First index of "<<target<<" is "<<indexOfFirstOccurence<<endl;


    //using Lower bound to find first occurence of element
